...
// This might be GXVertexAttribute
    ...
    {
        public List<OpenTK.Vector3> Position = new List<OpenTK.Vector3>();
        public List<OpenTK.Vector3> Normal = new List<OpenTK.Vector3>();
        public List<OpenTK.Vector3> Binormal = new List<OpenTK.Vector3>();
            ...
            for (int k = 0; k < vertexDataOffsets.Length; k++)
            {
                ...
                switch (k)
                {
                    ...
                    case 0:
                        ...
                        VertexData.Position = LoadVertexAttribute<OpenTK.Vector3>(reader, totalLength, vertexFormat.DecimalPoint, VertexArrayType.Position, vertexFormat.DataType, VertexColorType.None);
                        break;

                    // Normal Data
                    case 1:
                        vertexFormat = VertexFormats.Find(x => x.ArrayType == VertexArrayType.Normal);
                        VertexData.Normal = LoadVertexAttribute<OpenTK.Vector3>(reader, totalLength, vertexFormat.DecimalPoint, VertexArrayType.Normal, vertexFormat.DataType, VertexColorType.None);
                        break;

                    // Normal Binormal Tangent Data (presumed)
                    case 2:
                        vertexFormat = VertexFormats.Find(x => x.ArrayType == VertexArrayType.NormalBinormalTangent);
                        VertexData.Binormal = LoadVertexAttribute<OpenTK.Vector3>(reader, totalLength, vertexFormat.DecimalPoint, VertexArrayType.NormalBinormalTangent, vertexFormat.DataType, VertexColorType.None);
                        break;
