...
namespace JStudio.J3D.Animation
{
    ...
    public abstract class BaseJ3DAnimation
    {
        ...
        public static float kAnimFramerate = 30f;
        public float m_timeSinceStartedPlaying;
        protected bool m_isPlaying;

        public int TickCount;

        public bool OneTimeDone;
        ...
        public float TimeSinceStartedPlaying()
        {
            return m_timeSinceStartedPlaying;
        }

        public void SetTimeSinceStartedPlaying(float t)
        {
            m_timeSinceStartedPlaying = t;
        }
        ...
        public virtual bool Tick(float deltaTime)
        {
            if (m_isPlaying)
            {
                ...
                float animationFrame = m_timeSinceStartedPlaying * BCK.kAnimFramerate %
                                       AnimLengthInFrames;
                if (AnimLengthInFrames - animationFrame <= 0.5f)
                {
                    if (LoopMode == LoopType.Once) return false;
                }
            }
        ...
        public float GetAnimValue(List<Key> keys, float frameTime)
        {
            if (keys.Count == 0)
                return 0f;
        ...
        {
            return WMath.Lerp(key1.Value, key2.Value, t);
        }
