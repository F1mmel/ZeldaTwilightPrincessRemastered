...
inline void InitializeParticleLitSurfaceData(ParticleParams params, out SurfaceData outSurfaceData)
{
    ...
    // Apply texture offset directly in the texture sampling
    half2 uvOffset = _MainTex_ST.zw; // Example offset values, adjust as needed
    half4 offsetTexColor = _BaseMap.Sample(sampler_BaseMap, params.uv + uvOffset);
    albedo.rgb = offsetTexColor.rgb;
    normalTS = SampleNormalTS(params.uv + uvOffset, params.blendUv, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap), _BumpScale);
